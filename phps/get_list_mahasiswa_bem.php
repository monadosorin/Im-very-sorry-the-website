<?php
    ...
    $getFungsiosql = "SELECT * FROM bem_fungsionaris_2024 WHERE nrp = ?";
    $getFungsiostmt = $pdo->prepare($getFungsiosql);
    $getFungsiostmt->execute([$nrp]);
    //$getFungsiostmt->execute([$_GET['nrp']]);
    ...
    if (!isset($_POST['id'])) {
        ...
        if ($getFungsio['id_jabatan'] == 1) {
        ...
        } else if ($getFungsio['id_jabatan'] == 9 || $getFungsio['id_jabatan'] == 8) {
            // BENDUM  &&  SEKUM

            // mengambil anak departemen Bendum dan Sekum
            ...
            SELECT a.id, a.nama, a.nrp, c.nama as jabatan, a.id_bidang, b.singkatan as departemen FROM bem_fungsionaris_2024 a JOIN bem_departemen_2024 b ON a.id_departemen = b.id JOIN bem_jabatan_2024 c ON a.id_jabatan = c.id WHERE c.nama IN ('Kepala Departemen', 'Wakil Kepala Departemen') AND a.id_departemen = ?
            UNION
            SELECT a.id, a.nama, a.nrp, c.nama as jabatan, a.id_bidang, b.singkatan as departemen 
                FROM bem_fungsionaris_2024 a 
                JOIN bem_departemen_2024 b ON a.id_departemen = b.id 
                JOIN bem_jabatan_2024 c ON a.id_jabatan = c.id 
                WHERE a.id_departemen = ?
            ";
            $stmtFungsio = $pdo->prepare($sqlFungsio);
            $stmtFungsio->execute([$getFungsio['nrp'], $getFungsio['id_departemen'], $id_dept_anak]);
    ...
    // kebalikan dari assessment
        ...
        if ($getFungsio['id_jabatan'] == 1 || $getFungsio['id_jabatan'] == 2 || $getFungsio['id_jabatan'] == 3) {
            ...
            if ($getFungsio['id_bidang'] == 1) {
                ...
                } else if ($getFungsio['id_jabatan'] == 6) {
                    // ANGGOTA
            ...
            // INTERNSHIP
            $sqlFungsio = "SELECT a.id, a.nama, a.nrp, c.nama as jabatan, a.id_bidang, b.singkatan as departemen FROM bem_fungsionaris_2024 a LEFT JOIN bem_departemen_2024 b ON a.id_departemen = b.id JOIN bem_jabatan_2024 c ON a.id_jabatan = c.id WHERE c.nama IN ('Kepala Departemen', 'Wakil Kepala Departemen', 'Fungsionaris') AND a.id_departemen = ?";
            $stmtFungsio = $pdo->prepare($sqlFungsio);
            $stmtFungsio->execute([$getFungsio['id_departemen']]);
        }
    }
    ...
    while ($rowFungsio = $stmtFungsio->fetch()) {
        ...
        // Kuartal 1 tidak perlu penilaian ke intern. bem_kuartal1_2024
        if($rowFungsio['jabatan'] == 'Internship') {
            continue;
        } 
        //end
        ...
        if ($stmtCek->rowCount() == 0) {
            if ($rowFungsio['jabatan'] == 'Kepala Bidang' || $rowFungsio['jabatan'] == 'Wakil Kepala Bidang') {
                echo "<option value='" . $rowFungsio['id'] . "'>" . strtoupper($rowFungsio['nrp']) . " - " . $rowFungsio['nama'] . " - " . $rowFungsio['jabatan'] . " " . $rowFungsio['id_bidang'] . "</option>";
            } else if ($rowFungsio['jabatan'] == 'Kepala Departemen' || $rowFungsio['jabatan'] == 'Wakil Kepala Departemen' || $rowFungsio['jabatan'] == 'Fungsionaris' || $rowFungsio['jabatan'] == 'Internship') {
                echo "<option value='" . $rowFungsio['id'] . "'>" . strtoupper($rowFungsio['nrp']) . " - " . $rowFungsio['nama'] . " - " . $rowFungsio['jabatan'] . " " . $rowFungsio['departemen'] . "</option>";
            } else {
                echo "<option value='" . $rowFungsio['id'] . "'>" . strtoupper($rowFungsio['nrp']) . " - " . $rowFungsio['nama'] . " - " . $rowFungsio['jabatan'] . "</option>";
            }
        }
    }
?>