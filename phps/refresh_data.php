<?php
    ...
    if (isset($_GET['id'])) {
        if ($_GET['id'] == 1) {
            ...
            $sql = "SELECT a.id, b.nrp, b.nama, b.id_bidang, CASE
                    WHEN b.id_jabatan IN (1, 2, 3, 4, 5, 9) THEN d.nama
                    WHEN b.id_jabatan IN (6, 7, 8, 10, 11) THEN CONCAT(d.nama, ' ', c.singkatan)
                    END as jabatan, c.nama as departemen, a.testimoni, a.submitted_on, a.filtered_by, a.status, (SELECT DISTINCT f.nrp FROM bem_kuartal1_2024 e JOIN bem_fungsionaris_2024 f ON e.id_fungsionaris_penilai = f.id WHERE e.id_fungsionaris_penilai = a.id_fungsionaris_penilai) as nrp_penilai, (SELECT DISTINCT f.nama FROM bem_kuartal1_2024 e JOIN bem_fungsionaris_2024 f ON e.id_fungsionaris_penilai = f.id WHERE e.id_fungsionaris_penilai = a.id_fungsionaris_penilai) as nama_penilai FROM bem_kuartal1_2024 a JOIN bem_fungsionaris_2024 b ON a.id_fungsionaris = b.id LEFT JOIN bem_departemen_2024 c ON b.id_departemen = c.id JOIN bem_jabatan_2024 d ON b.id_jabatan = d.id WHERE a.status IN " . $status . " ORDER BY a.id DESC";
        } else if ($_GET['id'] == 14) {
